...
from models.process import Process, MemoryBlock, MemoryRequest
...
def build_memory_result(algorithm: str, blocks: List[MemoryBlock], requests: List[MemoryRequest]) -> Dict[str, Any]:
    ...
    return {
        ...
        'total_memory': total,
        'used_memory': sum(b.size for b in blocks if not b.is_allocated),
        'free_memory': sum(b.size for b in blocks if b.is_allocated),
        'memory_utilization': (sum(b.size for b in blocks if not b.is_allocated) / total * 100) if total else 0
    }
