...
def build_memory_result(algorithm: str, blocks: List[MemoryBlock], requests: List[MemoryRequest]) -> Dict[str, Any]:
    ...
    return {
        ...
        'free_memory': sum(b.size for b in blocks if b.is_free),
        'memory_utilization': (sum(b.size for b in blocks if not b.is_free) / total * 100) if total else 0
    }
...
def coalesce_free_blocks(blocks: List[MemoryBlock]):
    ...
    while i < len(blocks) - 1:
        current = blocks[i]
        nxt = blocks[i + 1]
        if current.is_free and nxt.is_free and current.end == nxt.start:
            current.end = nxt.end
