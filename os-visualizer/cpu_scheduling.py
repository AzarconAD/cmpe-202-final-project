...
def mlq(processes: List[Dict], quantum: int = 4, quantum_system: int = 4) -> Dict[str, Any]:
    procs = [Process(**p) for p in processes]
...
# ============= Algorithm Router =============
# Force every strategy mapping to safely accept and discard any excessive kwargs
ALGORITHMS = {
    'fcfs': lambda p, **kw: fcfs(p),
    'sjf': lambda p, **kw: sjf(p),
    'sjf_preemptive': lambda p, **kw: sjf_preemptive(p),
    'priority': lambda p, **kw: priority(p),
    'priority_preemptive': lambda p, **kw: priority_preemptive(p),
    'round_robin': lambda p, **kw: round_robin(p, kw.get('quantum', 4)),
    'mlq': lambda p, **kw: mlq(p, kw.get('quantum', 4), kw.get('quantum_system', 4)),
    'mlfq': lambda p, **kw: mlfq(p)
}
